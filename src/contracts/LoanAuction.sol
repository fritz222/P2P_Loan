...
contract LoanAuction is ERC721 {
    ...
    bool firsttime = false;  //to mark first successfull bid
   ...
   //Events
    ...
    struct LoanItem {
            ...
            uint256 minbid; //max rate set by lender
            string tokenURI;  //Prospect URL
   ...
   //modifiers
    ...
    //get Loan item info
    function getLoanItem(uint256 id) public view loanItemExist(id)
        returns (
            uint256,
            uint256,
            string memory,
    ...
    //Cancel auction
    function cancelAuction(uint256 id) public payable onlyOwner(id) onlyNotCanceled() returns (bool success){
        canceled = true;
   ...
   function placeBid(uint256 id) public payable onlyNotCanceled onlyNotOwner(id) minbid(id) returns (bool success){
